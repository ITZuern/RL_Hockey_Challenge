...
class Agent():
        ...
        if explore:
            if self.time_step < self.warmup:
                mu = T.tensor(np.random.normal(
                    scale=self.noise, size=(1, self.n_actions))).to(self.actor.device)
            else:
    ...
    def load(self, path):
        ...
        checkpoint_critic = T.load(path+"/critic1.pt")
        self.critic_1.load_state_dict(checkpoint_critic['model_state_dict'])

        checkpoint_critic = T.load(path+"/critic2.pt")
    ...
    def learn(self):
        ...
        # From the paper, add some noise for smoothing
